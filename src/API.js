...
export async function updateBlockAPI(text, blockid, textType = 'markdown') {
    ...
    try {
        if (
            response.code == 0 &&
            response.data != null &&
            isValidStr(response.data[0].doOperations[0].id)
        ) {
...
export async function getCurrentDocIdF() {
    ...
    if (isValidStr(thisWidgetId)) {
        let queryResult = await queryAPI(
            "SELECT root_id as parentId FROM blocks WHERE id = '" + thisWidgetId + "'"
        );
    ...
    try {
        if (isValidStr(thisWidgetId)) {
            ...
            let thisDocId = $(window.parent.document)
                .find(`div.protyle-content:has(.iframe[data-node-id="${thisWidgetId}"])`)
                .find(`.protyle-background`)
                .attr('data-node-id');
    ...
    if (!isValidStr(thisWidgetId)) {
        thisDocId = $(window.parent.document)
            .find('.layout__wnd--active .protyle.fn__flex-1:not(.fn__none) .protyle-background')
            .attr('data-node-id');
